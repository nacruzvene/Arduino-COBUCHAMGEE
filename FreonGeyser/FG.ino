...
/*TOP CHAMBER*/
#define TC_PELTSYS1 10
/*BOTTOM CHAMBER*/
#define TC_PELTSYS2 4

/*MIDDLE CHAMBER*/
#define TC_PELTSYS3 5


#define TC_PELTSYS4 11
...
#define fastPSampleInterval 100
...
int LastTriggerTime, pBufferTimeTracker, timeElapsedLastEvt;
...
void setup()
{
    ...
    pinMode(PELT_PWM4, OUTPUT);


    fastPressureBuffer.init(1024);

    /*Store the presure ADC cutoff value once in memory*/
    PressureADCCutoff = (cameraTriggerADCPSI *0.016/100.0 + 0.004 ) * 150.0 *1024.0 / 3.3;
...
void loop()
{
    ...
    register unsigned long currentMicros = micros();

    /*Fast pressure routine*/
    if(currentMicros - pBufferTimeTracker >= fastPSampleInterval){
        updateFastPressure(currentMicros);
        pBufferTimeTracker = currentMicros;
    }

    if(currentMillisecs - infoSweep_previousMillis > infoSweep_interval) {
        /*Save the last time information sweep happened*/
        SweepSenses();
        ReadTC_MUX(0);
